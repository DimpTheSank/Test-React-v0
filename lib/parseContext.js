export function renderContextBlock(raw, blockKey = 'c') {
  ...
  const parts = []
  const blockRegex = /\[(TABLE|BOX(?:[^\]]*)?|COL)\]([\s\S]*?)\[\/COL\]|\[(TABLE|BOX(?:[^\]]*)?)\]([\s\S]*?)\[\/(?:TABLE|BOX)\]/g
  // ⚠️ regex trên gộp 2 pattern khác nhau nên khó đọc — tách rõ ràng hơn ở dưới
  const tagRegex = /\[(TABLE|BOX(?:[^\]]*)?|COL)\]([\s\S]*?)\[\/(?:TABLE|BOX|COL)\]/g
  let lastIndex = 0
  let match
  ...
  while ((match = tagRegex.exec(raw)) !== null) {
    ...
    if (tag === 'TABLE') {
      ...
      parts.push({ type: 'col', content: match[2] })
    } else {
...
// ─── Column block (2-col layout cho bài báo) ─────────────────────────────────
    ...
    <div key={keyPrefix} className="ctx-2col" style={{
      ...
      {paragraphs.map((p, pai) => (
        <p key={`${keyPrefix}-p-${pai}`} style={{
          ...
          breakInside: 'avoid-column', // tránh cắt ngang 1 câu/đoạn giữa 2 cột nếu có thể
        }}>
