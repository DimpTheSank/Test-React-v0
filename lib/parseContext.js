    ...
    <div key={keyPrefix} className="ctx-2col" style={{
      ...
      {paragraphs.map((p, pai) => (
        <p key={`${keyPrefix}-p-${pai}`} style={{
          ...
          // Không set breakInside:avoid-column ở đây — nếu đoạn văn dài chỉ có 1 <p>
          // (không xuống dòng trống), avoid-column sẽ ép cả khối dồn vào 1 cột,
          // cột còn lại trống. Để mặc định cho phép ngắt dòng tự do giữa 2 cột.
        }}>
