export function renderContextBlock(raw, blockKey = 'c') {
  ...
  const parts = []
  const blockRegex = /\[(TABLE|BOX(?:[^\]]*)?|COLBREAK)\]([\s\S]*?)\[\/(?:TABLE|BOX|COLBREAK)\]|\[COLBREAK\]/g
  let lastIndex = 0
  let match
  ...
  while ((match = blockRegex.exec(raw)) !== null) {
    ...
    } else if (tag === 'COLBREAK' || match[0] === '[COLBREAK]') {
      parts.push({ type: 'colbreak' })
    } else {
  ...
  return parts.map((part, pi) => {
    ...
    if (part.type === 'colbreak') return <div key={key} style={{ breakBefore: 'column' }} />
